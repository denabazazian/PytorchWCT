    ...
    def transform_mask(self, cF, sF, cmasks, smasks, alpha):    
            ...
            for i in range(color_code_number):
                ...
                if(torch.sum(cmask) >= 10 and torch.sum(smask) >= 10 and not self.large_dff(torch.sum(cmask),torch.sum(smask))):
                    ...
                    bgcmask = (cmaskView == 0 ).nonzero().squeeze(1)

                    fgcmask = fgcmask.cuda()
                    cFView = cF.view(C,-1).cuda()
                    cFFG = torch.index_select(cFView,1,fgcmask.long())

                    bgcmask = bgcmask.cuda()
                    cFView = cF.view(C,-1).cuda()
                    cFBG = torch.index_select(cFView,1,bgcmask.long())
                    ...
                    #fgsmask = (smaskView > 1).nonzero().squeeze(1)
                    fgsmask = (smaskView > 0).nonzero().squeeze(1)
                    ...
                    targetFeatureBG = self.wct2(cFBG.float(),sFBG.float())
                    #pdb.set_trace()
                    targetFeature.index_copy_(1,fgcmask.cpu(),targetFeatureFG.cpu())
                    targetFeature.index_copy_(1,bgcmask.cpu(),targetFeatureBG.cpu())
                    #targetFeature.index_copy_(1,bgcmask.cpu(),cFBG.cpu())
                    del fgcmask
                    del bgcmask
                    del fgsmask
            targetFeature = targetFeature.view_as(cF)
            #targetFeatureBG = targetFeatureBG.view_as(cF)
            ccsF = alpha * targetFeature + (1.0 - alpha) * cF
            #ccsF = targetFeatureBG # alpha * targetFeature + (1.0 - alpha) *targetFeatureBG
            ccsF = ccsF.float().unsqueeze(0)
            return ccsF
